...
此版本为最稳定版本6.57版本，其中tianqi6.57为源代码。
...
* 6.5.7（三代微雪屏版）增加isAPActive标志，解决轻度休眠导致WIFI断开带来的配置时间过短问题。
* 6.5.6（三代微雪屏版）使用一个数组来存储每次成功获取的天气信息，并在成功更新天气信息时更新数组。如果获取天气信息失败，则使用数组中的上一次成功数据。
